 ...
 */
const changePassword = ({ usersDb }) => async (params) => {
  ...
  let user = await usersDb.findOne({ _id :params.userId });
  if (!user){
  ...
  let updatedUser = usersDb.updateOne({ _id : user.id }, { 'password':password });
  if (updatedUser) {
