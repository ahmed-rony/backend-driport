...
const updateReports = (updateReportsUsecase) => async (req,res) =>{
  try {
    if (!req.params.id){
...
const bulkUpdateReports = (bulkUpdateReportsUsecase) => async (req,res) => {
  try {
...
const partialUpdateReports = (partialUpdateReportsUsecase) => async (req,res) => {
  try {
    if (!req.params.id){
      return responseHandler(res,response.badRequest({ message : 'Insufficient request parameters! id is required.' }));
    }
...
const softDeleteReports = (softDeleteReportsUsecase) => async (req,res)=>{
  try {
    if (!req.params.id){
      return responseHandler(res,response.badRequest({ message : 'Insufficient request parameters! id is required.' }));
    }
...
const deleteReports = (deleteReportsUsecase) => async (req,res) => {
  try {
    if (!req.params.id){
      return responseHandler(res,response.badRequest({ message : 'Insufficient request parameters! id is required.' }));
    }
    let query = { _id: req.params.id };
    let result = await deleteReportsUsecase(query,req,res);
    return responseHandler(res,result);
  } catch (error){
    return responseHandler(res,response.internalServerError({ message:error.message }));
  }
...
const deleteManyReports = (deleteManyReportsUsecase) => async (req,res) => {
  try {
    ...
    let result = await deleteManyReportsUsecase(query,req,res);
    return responseHandler(res,result);
  } catch (error){
    return responseHandler(res,response.internalServerError({ message:error.message }));
  }
...
const softDeleteManyReports = (softDeleteManyReportsUsecase) => async (req,res) => {
  try {
    if (!req.body || !req.body.ids){
      return responseHandler(res,response.badRequest({ message : 'Insufficient request parameters! ids field is required.' }));
    }
